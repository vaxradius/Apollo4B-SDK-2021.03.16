...
char *conn_timeout_str[] = {
    NULL,
    NULL,
    "schedule flag NOT set ASAP",
    "schedule reach limited",
    "schedlue already started",
    "schedule reach limited 2",
};
            ...
            switch(trace_type)
            {
                ...
                case CUSTOM:
                {
                    ...
                    else if(custom_id == CUSTOM_ID_CONN_TIMEOUT_CAUSE)
                    {
                        ...
                        if(((custom_data >>8) & 0xFF)!=DBG_CONN_NO_ERROR)
                        {
                            am_util_debug_printf("Link %d connection timeout due to schedule fail: %s\r\n", link_id, conn_timeout_str[((custom_data >>8) & 0xFF)]);
                        }

                        if(((custom_data >>16) & 0xFF) !=DBG_CONN_NO_ERROR)
                        {
                            am_util_debug_printf("Link %d connection timeout, packet SYNC error\r\n", link_id);
                        }
                    }
                }
